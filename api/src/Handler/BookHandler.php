...
final class BookHandler implements MessageHandlerInterface
{
    ...
    public function __invoke(Book $book): void
    {
        ...
        $update = new Update(
            $this->iriConverter->getIriFromItem($book, 'get', UrlGeneratorInterface::ABS_URL),
            $this->serializer->serialize(
                $book,
                ItemNormalizer::FORMAT,
                $this->resourceMetadataCollectionFactory->create(Book::class)->getOperation('generate_cover')->getNormalizationContext() ?? []
            )
        );
